/*
 * Bezel boundary test — GeekMagic SmallTV Ultra
 *
 * 4면 각각 안쪽으로 5px 간격으로 선을 그립니다.
 *  TOP    → CYAN     수평선, y=0 부터 y=120
 *  BOTTOM → MAGENTA  수평선, y=239 부터 y=120
 *  LEFT   → GREEN    수직선, x=0 부터 x=120
 *  RIGHT  → YELLOW   수직선, x=239 부터 x=120
 *
 * 장치를 켜고 각 면에서 "처음으로 보이는 선"의 좌표를 기록하세요.
 *
 * WiFi 연결 성공 시 /update 엔드포인트로 OTA 복구 가능.
 *
 * 시리얼 플래시:  pio run -e esp8266_bezel -t upload --upload-port /dev/serial0
 * OTA 업데이트:   curl -F "image=@firmware.bin" http://<IP>/update
 */

#include <Arduino.h>
#include <ESP8266WiFi.h>
#include <ESP8266WebServer.h>
#include <ESP8266HTTPUpdateServer.h>
#include <TFT_eSPI.h>

static TFT_eSPI               tft;
static ESP8266WebServer        server(80);
static ESP8266HTTPUpdateServer updater;

#define W      240
#define H      240
#define BL_PIN   5
#define STEP     5
#define DEPTH   25

static const uint16_t COL_TOP_HI  = 0x07FF;  // CYAN
static const uint16_t COL_TOP_LO  = 0x0310;
static const uint16_t COL_BOT_HI  = 0xF81F;  // MAGENTA
static const uint16_t COL_BOT_LO  = 0x4008;
static const uint16_t COL_LFT_HI  = 0x07E0;  // GREEN
static const uint16_t COL_LFT_LO  = 0x0200;
static const uint16_t COL_RGT_HI  = 0xFFE0;  // YELLOW
static const uint16_t COL_RGT_LO  = 0x8400;

static void lbl(int x, int y, int val, uint16_t bg) {
    char buf[6];
    snprintf(buf, sizeof(buf), "%d", val);
    tft.setTextColor(TFT_WHITE, bg);
    tft.setTextSize(1);
    tft.setCursor(x, y);
    tft.print(buf);
}

static void drawPattern() {
    tft.fillScreen(TFT_BLACK);

    // TOP: horizontal lines y=0..120, labels left (x=3)
    for (int i = 0; i < DEPTH; i++) {
        int y = i * STEP;
        uint16_t c = (i % 2 == 0) ? COL_TOP_HI : COL_TOP_LO;
        tft.drawFastHLine(0, y, W, c);
        if (i % 2 == 0) lbl(3, y + 1, y, COL_TOP_HI);
    }

    // BOTTOM: horizontal lines y=239..115, labels right (x=195)
    for (int i = 0; i < DEPTH; i++) {
        int y = H - 1 - i * STEP;
        uint16_t c = (i % 2 == 0) ? COL_BOT_HI : COL_BOT_LO;
        tft.drawFastHLine(0, y, W, c);
        if (i % 2 == 0) lbl(195, y - 8, y, COL_BOT_HI);
    }

    // LEFT: vertical lines x=0..120, labels upper-center (y=108)
    for (int i = 0; i < DEPTH; i++) {
        int x = i * STEP;
        uint16_t c = (i % 2 == 0) ? COL_LFT_HI : COL_LFT_LO;
        tft.drawFastVLine(x, 0, H, c);
        if (i % 2 == 0) lbl(x + 1, 108, x, COL_LFT_HI);
    }

    // RIGHT: vertical lines x=239..115, labels lower-center (y=125)
    for (int i = 0; i < DEPTH; i++) {
        int x = W - 1 - i * STEP;
        uint16_t c = (i % 2 == 0) ? COL_RGT_HI : COL_RGT_LO;
        tft.drawFastVLine(x, 0, H, c);
        if (i % 2 == 0) {
            int lx = (x >= 18) ? x - 18 : x + 2;
            lbl(lx, 125, x, COL_RGT_HI);
        }
    }

    // 4 corner markers: 4x4 white
    tft.fillRect(0,     0,     4, 4, TFT_WHITE);
    tft.fillRect(W - 4, 0,     4, 4, TFT_WHITE);
    tft.fillRect(0,     H - 4, 4, 4, TFT_WHITE);
    tft.fillRect(W - 4, H - 4, 4, 4, TFT_WHITE);

    // Center crosshair
    tft.drawFastHLine(W/2 - 15, H/2, 30, TFT_WHITE);
    tft.drawFastVLine(W/2, H/2 - 15, 30, TFT_WHITE);
}

void setup() {
    Serial.begin(115200);
    delay(300);
    Serial.println("\n=== Bezel boundary test ===");

    // TFT init — same order as display.cpp
    tft.init();
    tft.setRotation(0);
    tft.fillScreen(TFT_BLACK);
    pinMode(BL_PIN, OUTPUT);
    analogWrite(BL_PIN, 220);

    drawPattern();

    // WiFi — connect with stored credentials only (no AP fallback)
    WiFi.mode(WIFI_STA);
    WiFi.begin();
    Serial.print("WiFi connecting");
    unsigned long t = millis();
    while (WiFi.status() != WL_CONNECTED && millis() - t < 10000) {
        delay(300); Serial.print('.');
        ESP.wdtFeed();
    }

    if (WiFi.status() == WL_CONNECTED) {
        Serial.printf("\nIP: %s\n", WiFi.localIP().toString().c_str());
        updater.setup(&server);   // mounts /update
        server.on("/", HTTP_GET, []() {
            server.send(200, "text/plain", "bezel-test fw — POST /update to reflash");
        });
        server.begin();

        // Show IP on display (bottom strip, small)
        tft.setTextColor(TFT_WHITE, TFT_BLACK);
        tft.setTextSize(1);
        tft.setCursor(60, 118);
        tft.print(WiFi.localIP().toString());
    } else {
        Serial.println("\nWiFi not connected (no saved creds?)");
    }

    Serial.println("Pattern drawn.");
    Serial.println("  TOP    (CYAN)    : left labels  → first visible y");
    Serial.println("  BOTTOM (MAGENTA) : right labels → first visible y");
    Serial.println("  LEFT   (GREEN)   : upper labels → first visible x");
    Serial.println("  RIGHT  (YELLOW)  : lower labels → first visible x");
}

void loop() {
    server.handleClient();
    ESP.wdtFeed();
    delay(10);
}
