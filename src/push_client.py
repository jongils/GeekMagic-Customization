...
def push_temp_to(device_ip: str, temp_c: float, timeout: int = 5) -> bool:
    """온도값을 지정 IP의 ESP8266으로 전송 (POST /temp {"c": XX})"""
    try:
        body = json.dumps({"c": round(temp_c, 1)}).encode()
        conn = http.client.HTTPConnection(device_ip, timeout=timeout)
        conn.request(
            "POST", "/temp", body=body,
            headers={
