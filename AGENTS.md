...
## 5. 안전 표시 영역 — y < 75 는 베젤에 가려짐

```
y = 0          ← 물리 패널 상단 (베젤에 가려짐)
y = 75         ← 안전 표시 영역 시작 (확인된 경계)
y = 239        ← 물리 패널 하단
```

좌/우/하단 베젤 여백은 `firmware/diag_bezel/` 테스트 펌웨어로 측정 중.
측정이 완료되면 이 파일과 `firmware/README.md`의 "표시 영역 주의사항" 섹션을 갱신할 것.
